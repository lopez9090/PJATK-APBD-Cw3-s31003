...
public sealed class ZadaniaLinq
{
    ...
    public IEnumerable<string> Zadanie04_PierwszyPrzedmiotAnalityczny()
    {
        ...
        return new[] { "Nie znaleziono żadnego przedmiotu z kategorii Analytics." };
    }
    ...
    {
        bool czyWszyscy = DaneUczelni.Prowadzacy.All(p => !string.IsNullOrWhiteSpace(p.Katedra));

        return new[] { $"Czy wszyscy mają katedrę? {(czyWszyscy ? "Tak" : "Nie")}" };
    }
    ...
    {
        int liczbaAktywnych = DaneUczelni.Zapisy.Count(z => z.CzyAktywny == true);
    ...
    {
        return DaneUczelni.Studenci
            .Join(DaneUczelni.Zapisy, s => s.Id, z => z.StudentId, (s, z) => new { Student = s, Zapis = z })
            .Where(x => x.Zapis.OcenaKoncowa.HasValue)
            .GroupBy(x => new { x.Student.Imie, x.Student.Nazwisko })
            .Select(g => $"{g.Key.Imie} {g.Key.Nazwisko} - Najwyższa ocena: {g.Max(x => x.Zapis.OcenaKoncowa.Value)}");
    }
    ...
    public IEnumerable<string> Wyzwanie02_PrzedmiotyStartujaceWKwietniuBezOcenKoncowych()
    {
        return DaneUczelni.Przedmioty
            ...
            .Join(DaneUczelni.Zapisy, p => p.Id, z => z.PrzedmiotId, (p, z) => new { Przedmiot = p, Zapis = z })
            .GroupBy(pz => pz.Przedmiot.Nazwa)
            .Where(g => g.All(pz => !pz.Zapis.OcenaKoncowa.HasValue))
            .Select(g => g.Key);
    }
    ...
    {
        return DaneUczelni.Prowadzacy
            .Join(DaneUczelni.Przedmioty, pr => pr.Id, p => p.ProwadzacyId, (pr, p) => new { Prowadzacy = pr, Przedmiot = p })
            .Join(DaneUczelni.Zapisy.Where(z => z.OcenaKoncowa.HasValue), pp => pp.Przedmiot.Id, z => z.PrzedmiotId, (pp, z) => new { pp.Prowadzacy, z.OcenaKoncowa })
            .GroupBy(pz => new { pz.Prowadzacy.Imie, pz.Prowadzacy.Nazwisko })
            .Select(g => $"{g.Key.Imie} {g.Key.Nazwisko} - Średnia: {g.Average(x => x.OcenaKoncowa.Value):F2}");
    }
